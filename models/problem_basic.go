...
func GetProblemList(keyword, categoryIdentity string) *gorm.DB {
	...
	/**
	tx := DB.Model(new(ProblemBasic)).Preload("ProblemCategories").preload("ProblemCategories.CategoryBasic) where
	*/
...
func GetProblemList2(keyword string) *gorm.DB {

	DB.Model(new(ProblemBasic)).
		Where("title like ? OR content like ?", "%"+keyword+"%", "%"+keyword+"%")
	data := make([]*Problem, 0)
	DB.Find(&data)
	for _, v := range data {
